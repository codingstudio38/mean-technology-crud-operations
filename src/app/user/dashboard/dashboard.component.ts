...
export class DashboardComponent implements OnInit {
  ...
  GetUserList(p: number, l: number) {
    ...
    this.APIservice.ForGetUserList(p, l).subscribe((response: HttpEvent<any>) => {
        ...
        case HttpEventType.Response:
          ...
          if (this.apibody.status == 200) {
            ...
            data.forEach((item: any, key: any) => {
              this.records.push({ '_id': item._id, 'name': item.name, 'phone': item.phone, 'email': item.email, 'photo': item.photo, 'slno': this.pagingCounter++, 'tokens': item.tokens, 'updated_at': item.updated_at, 'created_at': item.created_at, '__v': item.__v });
            });
  ...
  DeleteUser(id: any) {
    ...
    if (confirm("Are you sure ?")) {
      this.APIservice.ForDeleteUser(id).subscribe((response: HttpEvent<any>) => {
        switch (response.type) {
          case HttpEventType.Sent:
            //console.log('Sent' + HttpEventType.Sent);
            break;
